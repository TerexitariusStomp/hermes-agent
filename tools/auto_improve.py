...
def check_supermemory():
    """Check supermemory-enhanced memory system health."""
    try:
        sys.path.insert(0, AGENT_DIR)
        from tools.supermemory import get_stats, add_memory, inject_context
        
        stats = get_stats()
        log_improvement("supermemory_check", 
                       f"Memory: {stats.get('total_facts', 0)} facts, "
                       f"{stats.get('profile_preferences', 0)} preferences, "
                       f"{stats.get('contradictions', 0)} contradictions, "
                       f"{stats.get('recent_activity', 0)} recent activities")
        
        # Log a system health event to memory
        add_memory(f"System health check: {stats['total_facts']} facts tracked",
                  "session", "auto-improve", "system-health", 0.3)
        
        # Test context injection
        context = inject_context()
        if context:
            log_improvement("supermemory_context", 
                           f"Context preview: {context[:200]}")
        
        return True
    except Exception as e:
        log_improvement("supermemory_check", f"ERROR: {e}", "fail")
        return False
